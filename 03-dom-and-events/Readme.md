...
- The DOM represents your web page which is displayed as HTML or your browser window. The DOM lets us manipulate the page with JavaScript. It consists of nodes and objects that are in a tree.
The document being he root/trunk of the tree with HTML elements branching out from the document. 
...
[source](https://dev.to/karaluton/what-exactly-is-the-dom-jhg)

## How do we interact with the DOM?
First, the document needs to load before it can be interacted with. This means, any JavaScript file that will target the DOM must wait to execute after the dom itself has loaded.
This can be done in a few ways. One is by added the defer keyword to the script tag. Another is dropping the script tag to the bottom of the HTML document. The last is using window/document on load in your JavaScript file. 
```
...
To interact with DOM nodes we will use JavaScript methods and CSS selectors! 
**CSS Selectors**
